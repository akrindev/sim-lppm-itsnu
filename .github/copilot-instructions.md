...
- This project uses Tabler v1.4 for UI components and icons.
- Tabler provides a set of ready-to-use HTML components, icons, and utilities that can be integrated into Livewire and Blade templates.
- Use Tabler components when available for consistent styling and functionality.
- Use the search-docs tool to get documentation and code snippets for this project's version of Tabler.

### Tabler Component Creation & Reusability
- Create reusable Tabler components under `resources/views/components/tabler/` if they don't already exist.
- Before creating a new component, check existing components in `resources/views/components/tabler/` to avoid duplication.
- Tabler components should be combined with Alpine or Livewire interactivity when needed:
  - Use **Alpine** for lightweight, client-side interactions (animations, toggles, dropdowns, etc.)
  - Use **Livewire** for server-state interactions (form submissions, database operations, complex state management)
- When creating Blade components, **always search for existing Tabler components** in `resources/views/components/tabler/` before writing raw HTML.
- Document the purpose and available props for each component within the component file using PHPDoc or comments.
- Keep components focused and single-purpose for maximum reusability across the application.
