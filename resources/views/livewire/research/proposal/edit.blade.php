...
<div>
                            ...
                            <select id="research_scheme"
                                ...
                                @foreach ($this->schemes as $scheme)
                                    <option value="{{ $scheme->id }}"
                                        {{ $form->research_scheme_id == $scheme->id ? 'selected' : '' }}>
                                        {{ $scheme->name }}</option>
                                @endforeach
                            </select>
                            ...
                            <select id="focus_area"
                                ...
                                @foreach ($this->focusAreas as $area)
                                    <option value="{{ $area->id }}"
                                        {{ $form->focus_area_id == $area->id ? 'selected' : '' }}>{{ $area->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select id="theme"
                                ...
                                @foreach ($this->themes as $theme)
                                    <option value="{{ $theme->id }}"
                                        {{ $form->theme_id == $theme->id ? 'selected' : '' }}>{{ $theme->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('form.theme_id')
                            ...
                            <select id="topic"
                                ...
                                @foreach ($this->topics as $topic)
                                    <option value="{{ $topic->id }}"
                                        {{ $form->topic_id == $topic->id ? 'selected' : '' }}>{{ $topic->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('form.topic_id')
                            ...
                            <select id="cluster_level1"
                                ...
                                @foreach ($this->scienceClusters->where('level', 1) as $cluster)
                                    <option value="{{ $cluster->id }}"
                                        {{ $form->cluster_level1_id == $cluster->id ? 'selected' : '' }}>
                                        {{ $cluster->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select id="cluster_level2"
                                ...
                                @foreach ($this->scienceClusters->where('level', 2) as $cluster)
                                    <option value="{{ $cluster->id }}"
                                        {{ $form->cluster_level2_id == $cluster->id ? 'selected' : '' }}>
                                        {{ $cluster->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select id="cluster_level3"
                                ...
                                @foreach ($this->scienceClusters->where('level', 3) as $cluster)
                                    <option value="{{ $cluster->id }}"
                                        {{ $form->cluster_level3_id == $cluster->id ? 'selected' : '' }}>
                                        {{ $cluster->name }}</option>
                                @endforeach
                            </select>
    ...
    @script
        <script>
            // Initialize Tom Select with proper values after Livewire hydration
            $wire.on('$refresh', () => {
                setTimeout(() => {
            ...
            // On component mount, ensure Tom Select has the correct values
            setTimeout(() => {
                document.querySelectorAll('select.tom-select').forEach((selectEl) => {
                    if (selectEl.tomSelect) {
                        const currentValue = selectEl.value;
